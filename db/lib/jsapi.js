 ...
 * Generated for database: https://birthday-b8b2.restdb.io
 * Date: Wed Apr 22 2020 12:34:12 GMT+0200 (CEST)
 */
...
var restdb = (function(apikey, opt) {
	...
	this._url = opt.url || "https://birthday-b8b2.restdb.io";
	this._apikey = apikey;
	...
	setInterval(function(){
		...
		if (_pingdiff > _keepalivediff) {
			_conn();
			$(document).trigger("birthdayb8b2_reconnect_event");
		}
	}, _keepalivedelay);
	...
	var _on = function(evt, callback){
		...
		switch (evt) {
			case "CONNECT":
				$(document).on("birthdayb8b2_connect_event",function(e, data){
					_log("Connect");
					if (callback){callback(null, null);}
				});
				break;
			case "DISCONNECT":
				$(document).on("birthdayb8b2_disconnect_event",function(e, data){
					_log("Disconnect");
					if (callback){callback(null, null);}
				});
				break;
			case "RECONNECT":
				$(document).on("birthdayb8b2_reconnect_event",function(e, data){
					_log("Reconnect");
					if (callback){callback(null, null);}
				});
				break;
			case "POST":
				$(document).on("birthdayb8b2_post_data_event",function(e, data){
					_log("Post");
					if (callback){callback(null, data);}
				});
				break;
			case "PUT":
				$(document).on("birthdayb8b2_put_data_event",function(e, data){
					_log("Put");
					if (callback){callback(null, data);}
				});
				break;
			case "DELETE":
				$(document).on("birthdayb8b2_delete_data_event",function(e, data){
					_log("Delete");
					if (callback){callback(null, data);}
				});
				break;
			default:
				$(document).on("birthdayb8b2_publish_data_event",function(e, data){
					_log("Publish");
	...
	var _conn = function() {
		...
		_es.addEventListener("open", function(e) {
			_log("Realtime open");
			$(document).trigger("birthdayb8b2_connect_event");
		});
		...
		_es.addEventListener('put', function(e) {
			...
			_log("PUT evt ", eo);
			$(document).trigger("birthdayb8b2_put_data_event", eo);
		}, false);
		_es.addEventListener('post', function(e) {
			var eo = JSON.parse(e.data);
			_log("POST evt ", eo);
			$(document).trigger("birthdayb8b2_post_data_event", eo);
		}, false);
		_es.addEventListener('delete', function(e) {
			var eo = JSON.parse(e.data);
			_log("DELETE evt ", eo);
			$(document).trigger("birthdayb8b2_delete_data_event", eo);
		}, false);
		_es.addEventListener('publish', function(e) {
			var eo = JSON.parse(e.data);
			_log("DELETE evt ", eo);
			$(document).trigger("birthdayb8b2_publish_data_event", eo);
		}, false);
		_es.addEventListener("error", function(e) {
			_log('Connect error ...',e);
			var txt;
			$(document).trigger("birthdayb8b2_disconnect_event");
	...
	*/
	...
	wishes.prototype = {
		...
		save: function(callback) {
			// save to db
			var url = _self._url + "/rest/wishes";
			var json = {};
			if (this['_id']) {
				json['_id'] = this['_id'];
				url += "/" + this['_id'];
		...
		delete: function(callback) {
			// delete from db
			var url = _self._url + "/rest/wishes/"+this['_id'];
			_log("Delete: ", url)
			if (this['_id']) {
				_ajax({url : url, type : 'DELETE', retry: true}, callback);
			}
		...
		on: function(evt, callback) {
			...
			var _that = this;
			$(document).on("birthdayb8b2_put_data_event",function(e, data){
				if ("wishes" === data.collection && (evt === "PUT" || evt === "DELETE")){
					if (_that['_id'] && _that['_id'] === data.data){
						_log("wishes this Observer: ",data);
						if (callback){callback(null, _that);}
					}
				}
			});
	...
	// Query
	wishes.find = function(query, hint, callback){
		// find by query
		var url = _self._url + "/rest/wishes?q="+JSON.stringify(query);
		if (hint && hint.metafields) {
			url += "&metafields=true";
			delete hint.metafields;
		}
		...
		_ajax({url : url, type : 'GET', retry: true}, function(_err, _res, _status, xhr){
			if (callback && !_err){
				...
				if(hint["$aggregate"] || hint["$groupby"]) {
					...
					for (index = 0, len = _res.length; index < len; ++index) {
						arr.push( new wishes(_res[index]));
					}
					callback(null, arr);
				}
	...
	// Get object by ID
	wishes.getById = function(ID, callback){
		// find by ID
		var url = _self._url + "/rest/wishes/" + ID;
		_log("getById: ", url);
		_ajax({url : url, type : 'GET', retry: true}, function(err, res){
			if (callback && !err) {
				callback(null, new wishes(res));
			} else {
				if (callback){
					callback(err, null);
				}
	...
	// listen for collection events
	wishes.on = function(evt, callback){
		if (!opt.realtime) {return};
		// listen for realtime events
		$(document).on("birthdayb8b2_put_data_event",function(e, data){
			if ("wishes" === data.collection && evt === "PUT"){
				_log("wishes Observer put: ",data);
				if (callback){callback(null, data);}
			}
		});
		$(document).on("birthdayb8b2_post_data_event",function(e, data){
			if ("wishes" === data.collection && evt === "POST"){
				_log("wishes Observer post: ",data);
				if (callback){callback(null, data);}
			}
		});
		$(document).on("birthdayb8b2_delete_data_event",function(e, data){
			if ("wishes" === data.collection && evt === "DELETE"){
				_log("wishes Observer delete: ",data);
				if (callback){callback(null, data);}
			}
		});
	};
